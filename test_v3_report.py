...
"""
import pandas as pd
from business_semantic_report_v3 import generate_business_semantic_report

def test_v3_report():
    """测试V3版本报告生成"""
    print("="*80)
    print("测试业务语义分析报告生成器 V3")
    print("="*80)
    ...
    # 保存报告
    with open('business_report_v3.md', 'w', encoding='utf-8') as f:
        f.write(md_report)
    print("\n✓ 已保存: business_report_v3.md")
    
    with open('business_report_v3.docx', 'wb') as f:
        f.write(word_report)
    print("✓ 已保存: business_report_v3.docx")
    
    # 显示报告预览
    print("\n" + "="*80)
    print("报告预览（前2500字符）:")
    print("="*80)
    print(md_report[:2500])
