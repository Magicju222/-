...
class KeyInsight:
    ...
    supporting_table: Optional[pd.DataFrame] = None  # 支撑数据表格
    confidence: str = "中"
    priority: str = "中"
...
class BusinessSemanticReportGeneratorV3:
    """
    业务语义分析报告生成器 V3
    ...
    def _extract_key_insights(self) -> List[KeyInsight]:
        ...
        for analysis in self.dimension_analyses:
            if analysis.top_values:
                ...
                # 创建支撑数据表格
                support_table = analysis.data.copy()
                support_table['占比(%)'] = (support_table['汇总'] / total * 100).round(2)
                support_table['排名'] = range(1, len(support_table) + 1)
                ...
                insight = KeyInsight(
                    ...
                    supporting_table=support_table,
                    confidence="高",
                    priority="高"
                )
        ...
        if self.business_metrics:
            ...
            if sum_metrics:
                # 使用循环代替lambda
                max_metric = sum_metrics[0]
                for m in sum_metrics[1:]:
                    if m.value > max_metric.value:
                        max_metric = m
                ...
                insight = KeyInsight(
                    ...
                    supporting_table=metrics_table,
                    confidence="高",
                    priority="中"
                )
                insights.append(insight)
        ...
        if len(insights) < 2:
            ...
            overview_table = pd.DataFrame([
                {'项目': '总记录数', '数值': len(self.df), '说明': '数据总行数'},
                {'项目': '数值指标数', '数值': len(self.numeric_columns), '说明': '可量化的业务指标'},
                {'项目': '分类维度数', '数值': len(self.categorical_columns), '说明': '可用于分组的维度'},
                {'项目': '时间维度数', '数值': len(self.datetime_columns), '说明': '可用于趋势分析的时间字段'}
            ])
            ...
            insight = KeyInsight(
                ...
                supporting_table=overview_table,
                confidence="高",
                priority="中"
            )
            insights.append(insight)
    ...
    def generate_markdown_report(self, title: str = "业务分析报告") -> str:
        ...
        if self.dimension_analyses:
            for i, analysis in enumerate(self.dimension_analyses[:5], 1):
                ...
                report_lines.append(self._format_table_markdown(dim_table, f"{analysis.dimension_name}维度详细数据"))
                
                # 添加详细说明
                if analysis.top_values:
                    top1 = analysis.top_values[0]
                    report_lines.append(f"**详细说明**：")
                    report_lines.append(f"- 表现最优：{top1.get(analysis.dimension_name, '未知')}，汇总值{top1.get('汇总', 0):.2f}")
                    if len(analysis.top_values) > 1:
                        top2 = analysis.top_values[1]
                        report_lines.append(f"- 排名第二：{top2.get(analysis.dimension_name, '未知')}，汇总值{top2.get('汇总', 0):.2f}")
                    report_lines.append(f"- 数据覆盖：共{len(analysis.data)}个类别，平均每个类别{analysis.data['平均'].mean():.2f}")
                    report_lines.append("")
        ...
        report_lines.append("")
        report_lines.append("本章节基于数据分析提取关键业务洞察，每个洞察都有完整的数据表格支撑。")
        report_lines.append("")
        ...
        if self.key_insights:
            for i, insight in enumerate(self.key_insights, 1):
                ...
                if insight.supporting_table is not None and not insight.supporting_table.empty:
                    report_lines.append(self._format_table_markdown(insight.supporting_table, "支撑数据详情"))
                elif insight.supporting_data:
                    # 如果没有表格，将supporting_data转换为表格
                    support_df = pd.DataFrame([insight.supporting_data])
                    report_lines.append(self._format_table_markdown(support_df, "支撑数据详情"))
...
def generate_business_semantic_report(df: pd.DataFrame, title: str = "业务分析报告") -> Tuple[str, bytes]:
    """生成业务语义分析报告的便捷函数"""
    generator = BusinessSemanticReportGeneratorV3(df)
    
    md_report = generator.generate_markdown_report(title=title)
    word_buffer = generator.generate_word_report(title=title)
    word_report = word_buffer.getvalue()
